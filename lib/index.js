...
function returnVersion(dependencies) {
	...
	return Object.keys(dependencies).map(async pkg => {
		...
		try {
			if (isNaN(parseInt(actualVersion[0]))) {
				return {
					name: pkg,
					actualVersion,
					lastVersion: '0.0.0'
...
function prepareToPrint(promisesArray, projectName) {
	...
	return Promise.all(promisesArray).then(values => {
		spinner.text = chalk`Start parsing {green ${projectName}/} folder dependencies from {green ${path.resolve(
			input[0]
		)}/}`;
...
async function checkDependencies(pkg, project) {
	...
	if (pkg.dependencies && !pkg.devDependencies) dependencies = pkg.dependencies;
	if (!pkg.dependencies && pkg.devDependencies) dependencies = pkg.devDependencies;
	if (pkg.dependencies && pkg.devDependencies) dependencies = { ...pkg.dependencies, ...pkg.devDependencies };

	return prepareToPrint(
		[...returnVersion(dependencies)],
		project
	);
}
