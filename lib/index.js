	...
	return Object.keys(dependencies).map(async pkg => {
		...
		allias.map(item => {
			if (actualVersion.includes(item)) {
				actualVersion = actualVersion.split(item)[1];
				return true;
			}
			return false;
		});
...
function prepareToPrint(promisesArray, projectName) {
	...
	return Promise.all(promisesArray).then(values => {
		spinner.text = chalk`Start parsing {green ${projectName}/} dependencies from {green ${path.resolve(input[0])}/}`;
...
async function checkDependencies(pkg, project) {
	...
	if (pkg.dependencies && !pkg.devDependencies) {
		return await prepareToPrint([...returnVersion(pkg.dependencies)], project);
	}

	// Has devDependencies but 0 dependencies
	if (!pkg.dependencies && pkg.devDependencies) {
		return await prepareToPrint([...returnVersion(pkg.devDependencies)], project);
	}

	// Has dev and dependencies
	if (pkg.dependencies && pkg.devDependencies) {
		return await prepareToPrint(
			[
				...returnVersion(pkg.dependencies),
				...returnVersion(pkg.devDependencies)
			],
			project
		);
	}
...
async function run(pathEnter) {
	...
	try {
		...
		if (directory.includes("package.json")) {
			...
			print = directory.map(async function(item) {
		...
		Promise.all(print).then(function(values) {
			console.log(print);
			spinner.succeed("Everything works well, now what do you want a do ?");
			// TODO: Display the data in a good format.
			// TODO: Check if data only contains false return a `spineer.fail()`
			console.log(textTable(...values));
		});
	} catch (error) {
